...
The figure uses the shared solver from stop_model.py. Expected wait is computed
with Little's law using boarding plus reneging as exits. The Poisson and perfectly
punctual calculations are displayed as two benchmark interpretations of the same
mean headway, not as universal probabilistic bounds.
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
for name,lo,hi in bands:
    ...
    waits.append(L/(thr+ren))                     # Little's law with boarding + reneging exits
...
ax1.bar(x, p15_sched, width=0.6, facecolor='none', edgecolor="#5B7FBF",
        ...
        label="punctual-schedule benchmark  min(15/h, 1)")
...
plt.title("Time-of-day service reliability — First Glasgow route 77, Partick\n"
          "(real timetable, Tue 23 Jun 2026; solid = Poisson benchmark, dashed = punctual benchmark)",
          fontsize=11)
